...
##################################################
#####바퀴, 킥/점프 및 드리블 모드와 관련된 변수 계산#####
#####규칙기반 알고리즘의 제어 부분 정의, 컨트롤러#####
##################################################
import os
import sys
...
class ActionControl:
    ...
    def update_state_2(self, cur_posture, cur_ball, cur_posture_opp):
        self.cur_posture = cur_posture
        self.cur_ball = cur_ball
        self.cur_posture_opp = cur_posture_opp

    def go_to(self, robot_id, x, y):    #로봇이 (x, y)로 이동
        sign = 1
        kd = 7 if ((robot_id == 1) or (robot_id == 2)) else 5   #거리 편차 위한 상수
        ka = 0.3                                                #각도 편차 위한 상수
        ...
        d_th = helper.wrap_to_pi(desired_th - self.cur_posture[robot_id][TH])   #각도 편차
    ...
    def go_fast(self, robot_id):    #상황이 느려지지 않을 경우 True반환, go_to()에서 대상에 가까울수록 속도 느려짐
    ...
    def kick(self, cross, shoot, quickpass):    #선택한 kick에 따라(cross, shoot, quickpass) 킥속도와 킥각도 반환
                                                #cross=높게 차는 킥, shoot=낮고 빠르게 차는 킥, quickpass=최고 속도의 절반으로
    ...
    def jump(self, flag):   #flag에 따라 점프 속도(최대속도) 반환
        # return jump speed
        # range: 0-10
        if flag:
            return 10
    ...
    def dribble(self, flag):    #ture/false에 따라 드리블 모드 전환(on/off)
