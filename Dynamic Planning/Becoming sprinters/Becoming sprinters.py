...
class Player(Participant):
    def init(self, info):
        ...
        self.GK = Goalkeeper(self.field, self.goal, self.penalty_area, 
                                ...
                                self.max_linear_velocity)   #class개체를 만들때 players.py의 각 class의 __init__함수 호출
...
##############################################################################
            ...
            self.set_speeds(self.speeds)    #모든 속도를 수신받은 후에 simulator로 보냄
##############################################################################

            self.previous_frame = received_frame    #현재 프레임을 이전 프레임으로 업데이트
            self.previous_ball = self.cur_ball
