...
try:
    ...
    print('standard_role-based_players: \'participant\' module cannot be imported:', err)
    raise
    ...
    def move(self, idx, idx_opp, defense_angle, attack_angle, cur_posture, cur_posture_opp, 
        ...
        if game_state == STATE_DEFAULT:
            ...
            if helper.ball_is_gk_zone(predicted_ball, self.field, self.goal_area):
                ...
                # D1
                d1_control = self.action.go_to(self.d1_index, d1_protection_x, d1_protection_y)
                # D2
                d2_control = self.action.go_to(self.d2_index, math.cos(defense_angle) * 0.5 + predicted_ball[X], -self.field[Y]/2 + 0.5)
                # F1
                f1_control = self.action.go_to(self.f1_index, math.cos(defense_angle) * 0.5 + predicted_ball[X], self.field[Y]/2 - 0.5)
                # F2
                f2_control = self.action.go_to(self.f2_index, 0, 0)
                self.flag = 1
            # D1 ZONE
                ...
                if (self.d1_index == idx):
                    if (cur_posture[self.d1_index][BALL_POSSESSION]):
                        if self.pass_count == 0:
                            ...
                            self.pass_count += 1
                        d1_control = self.action.pass_to(self.d1_index, cur_posture[self.d1_target_robot_id][X], cur_posture[self.d1_target_robot_id][Y])
                    else:
                        if (cur_posture[self.d1_index][X] > predicted_ball[X]):
                            d1_control = self.action.go_to(self.d1_index, -math.cos(defense_angle) * 0.2 + predicted_ball[X], -math.sin(defense_angle) * 0.2 + predicted_ball[Y])
                        else:
                            d1_min_x = -self.field[X]/2 + self.goal_area[X] + 0.1
                            d1_control = self.action.go_to(self.d1_index, max(d1_min_x, predicted_ball[X]), predicted_ball[Y])
                else:
                    d1_control = self.action.go_to(self.d1_index, d1_protection_x, d1_protection_y)
                # D2
                d2_control = self.action.go_to(self.d2_index, math.cos(defense_angle) * 0.5 + predicted_ball[X], -self.field[Y]/2 + 0.5)
            ...
            # D2 ZONE
                ...
                if (cur_posture[self.d2_index][BALL_POSSESSION]):
                    if self.pass_count == 0:
                        ...
                        self.pass_count += 1
                    d2_control = self.action.pass_to(self.d2_index, cur_posture[self.d2_target_robot_id][X], cur_posture[self.d2_target_robot_id][Y])
                else:
                    if (cur_posture[self.d2_index][X] > predicted_ball[X]):
                        d2_control = self.action.go_to(self.d2_index, -math.cos(defense_angle) * 0.2 + predicted_ball[X], -math.sin(defense_angle) * 0.2 + predicted_ball[Y])
                    else:
                        d2_min_x = -self.field[X]/2 + self.goal_area[X] + 0.1
                        d2_control = self.action.go_to(self.d2_index, max(d2_min_x, predicted_ball[X]), predicted_ball[Y])                        
                # F1
                f1_control = self.action.go_to(self.f1_index, math.cos(defense_angle) * 1.5 + predicted_ball[X], self.field[Y]/2 - 0.5)
                # F2
                if cur_ball[X] < 0:
                    f2_control = self.action.go_to(self.f2_index, 0.5, 0)
                else:
                    f2_control = self.action.go_to(self.f2_index, self.field[X]/2 - self.penalty_area[X] - 1.0, 0)
                self.flag = 3
            # F1 ZONE
                ...
                d1_control = self.action.go_to(self.d1_index, d1_protection_x, d1_protection_y)
                # D2
                d2_control = self.action.go_to(self.d2_index, math.cos(defense_angle) * 1.5 + predicted_ball[X], -self.field[Y]/2 + 0.5)
                # F1
                if (cur_posture[self.f1_index][BALL_POSSESSION]):
                    if self.cross_count == 0:
                        ...
                        self.cross_count += 1
                    f1_control = self.action.cross_to(self.f1_index, cur_posture[self.f1_target_robot_id][X], cur_posture[self.f1_target_robot_id][Y], self.robot_height)
                    if f1_control == None:
                    ...
                    if (cur_posture[self.f1_index][X] > predicted_ball[X]):
                        f1_control = self.action.go_to(self.f1_index, -math.cos(defense_angle) * 0.2 + predicted_ball[X], -math.sin(defense_angle) * 0.2 + predicted_ball[Y])
                    else:
                        f1_min_x = -self.field[X]/2 + self.goal_area[X] + 0.1
                        f1_control = self.action.go_to(self.f1_index, max(f1_min_x, predicted_ball[X]), predicted_ball[Y])
                # F2     
                if cur_ball[X] < 0:
                    f2_control = self.action.go_to(self.f2_index, 0.5, 0)
                else:
            ...
            # F2 ZONE
                ...
                d1_control = self.action.go_to(self.d1_index, d1_protection_x, d1_protection_y)
                # D2
                d2_control = self.action.go_to(self.d2_index, -math.cos(attack_angle) * 1.5 + predicted_ball[X], -math.sin(attack_angle) * 1 + predicted_ball[Y])
                # F1
                f1_control = self.action.go_to(self.f1_index, -math.cos(attack_angle) * 1.5 + predicted_ball[X], math.sin(attack_angle) * 1 + predicted_ball[Y])
                # F2
                if (cur_posture[self.f2_index][BALL_POSSESSION]):
                        self.shoot_count += 1
                        f2_control = self.action.shoot_to(self.f2_index, self.field[X]/2, 0)
                else:
