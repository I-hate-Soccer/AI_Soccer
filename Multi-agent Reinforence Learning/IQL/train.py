...
import helper
from dqn import DQN
from rl_utils import get_action, get_reward, get_state, Logger, TouchCounter
...
class Player(Participant):
    ...
    def update(self, received_frame):
        ...
        if received_frame.end_of_frame:
            ...
            self.reward = [get_reward(self.cur_posture, self.prev_posture, self.cur_ball, self.prev_ball, self.field, i, self.touch_counter, received_frame.reset_reason) for i in range(self.number_of_robots)]
            ...
            if (received_frame.reset_reason > 1) :
                ...
                if self.t >= 10:
                    ...
                    if self.episode % self.save_png_interval == 0:
                        self.plot_reward.plot('IQL-REWARDS')
                    self.episode += 1
